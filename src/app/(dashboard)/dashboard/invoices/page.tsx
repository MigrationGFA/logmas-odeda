...
import { PageHeader, StatusBadge, StatCard } from "@/components/dashboard/shared";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Plus, Search, Download, Eye, Clock, TrendingUp, CreditCard, Wallet, Loader2 } from "lucide-react";
...
export default function InvoicesPage() {
  ...
  const filtered = invoices.filter((i) =>
    (tab === "all" || i.status === tab || (tab === "unpaid" && i.status === "sent")) &&
    (i.reference + i.customerName + i.levyType).toLowerCase().includes(q.toLowerCase())
  );
  ...
  return (
    <div>
      <PageHeader
        ...
        action={(user?.role === "field_officer" || user?.role === "lga_admin") && (
                    ...
                    <div className="flex justify-end gap-1">
                      ...
                      {invoice.status === "paid" && invoice.receiptId ? (
                        <Button asChild variant="outline" size="sm">
                          <Link href={`/dashboard/receipts/${invoice.receiptId}`}>
                            Receipt
                          </Link>
                        </Button>
                      ) : invoice.status !== "paid" ? (
                        <Button asChild size="sm" className="bg-gradient-hero">
                          <Link href={`/dashboard/invoices/${invoice.reference}`}>
                            Pay now
                          </Link>
                        </Button>
                      ) : null}
